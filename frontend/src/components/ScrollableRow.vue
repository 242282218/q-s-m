...
function scrollRight() {
  ...
  const cardWidth = firstChild?.getBoundingClientRect().width || 200;
  const gap = getComputedStyleValue('--spacing-4') || 16;
...
<template>
  <div class="scrollable-row-wrapper">
    ...
    <button
      ...
      :aria-label="ariaLabel ? `向左滑动${ariaLabel}` : '向左滑动'"
      @click="scrollToLeft"
    >
      <IconChevronLeft />
    </button>
    ...
    <div
      ref="scrollContainer"
      class="scrollable-row-container"
      tabindex="0"
      role="region"
    ...
    <button
      ...
      :aria-label="ariaLabel ? `向右滑动${ariaLabel}` : '向右滑动'"
      @click="scrollRight"
    >
      <IconChevronRight />
    </button>
