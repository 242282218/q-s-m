...
import { enhancedCache, getCacheStats } from '@/api';
import { globalDeduplicator } from '@/composables/useApiCache';
  ...
  describe('Combined Performance Benefits with Error Handling', () => {
    it('should continue operating despite individual component errors', async () => {
      ...
      const requests = Array(5)
        .fill(null)
        .map(async () => {
          return globalDeduplicator.getOrCreate('api-result', async () => {
      ...
      // 显式读取一次缓存，验证命中统计。
      expect(enhancedCache.get('api-result')).toEqual(results[0]);
