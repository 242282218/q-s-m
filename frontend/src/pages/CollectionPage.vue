...
function onSseEvent(envelope: SseEnvelope) {
  ...
  // 从 envelope 中获取 level，这才是正确的日志级别
  const level = String(envelope.level || 'info');
  applyProgress(event);
...
async function onRename(item: CollectionItem) {
  ...
  try {
    await startRenameSse(
      ...
      (envelope) => {
        // 检查是否是错误事件
        if (envelope.type === 'error' || envelope.code === 500) {
          const message = envelope.message || envelope.data.message || '任务失败';
          appendLine('error', message);
          modalSummary.value = message;
          push(message, 'error');
          return;
...
<template>
  <div class="page page-padded collection-page">
      ...
      <div class="section-header collection-header">
        ...
        <div class="collection-actions">
          <button
            ...
            @click="loadCollections(pagination.page)"
          >
            刷新
          </button>
          <button
            class="btn btn-primary collection-verify-btn"
            :disabled="loading || taskBusy"
            @click="onVerifyAll"
          >
            验证网盘状态
            ...
            <div class="card-actions-overlay">
              <div class="card-actions">
                ...
                <button
                  class="card-action-btn"
                  :class="{
                    'is-disabled': item.status !== 1,
                    'is-busy': busyAction[item.id] === 'rename',
                  }"
...
<style scoped>
/* FIXED: 移除硬编码的 padding，使用 CSS 变量确保响应式布局 */
.collection-page {
  padding-top: calc(var(--site-header-height, 72px) + var(--spacing-8));
...
@media (max-width: 767px) {
  .collection-page {
    padding-top: calc(var(--site-header-height, 64px) + var(--spacing-4));
    padding-bottom: var(--spacing-12);
  }

  .card-actions-overlay {
    opacity: 1;
    background: linear-gradient(180deg, transparent 50%, var(--color-overlay-dark) 100%);
